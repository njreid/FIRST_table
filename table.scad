...
module corner_tab() {
  fwd(w / 2) rotate([closed * 45, 0, 0]) fwd(cl / 2) {
        ...
        fwd(cl / 2) rotate([closed * 45, 0, 0]) fwd(tl / 2) color("blue") cubh([wh - 2 * t, tl, 0.25], center=true);
      }
}
...
// stretchers for reinforcing under the table, (tabs extend up the exterior of the walls)
sc = [0.5, 0.4, 0.1];
sw = 3; // stretcher width
sh = wh + t;
sl = w + 2 * ww + 2 * t;

module stretcher(c) {
  color(c) cubh([sw, sl, t], center=true);
  color("blue") fwd((sl + sh) / 2) cubh([sw, sh, 0.25], center=true);
  color("blue") back((sl + sh) / 2) cubh([sw, sh, 0.25], center=true);
}

// start of extra space
xtr = l / 2 + sw / 2;

fwd(6 - sh - t) right(xtr) {
    for (i = [0:1:3]) {
      right(i * sw) stretcher(c=[sc[0], sc[1], sc[2] + (i % 2) * 0.6]);
    }
  }
